...
![20180529001324885](./pic/20180529001324885.jpg)
...
客户端最后一次向服务器回传 ``` ACK ``` 包时，有可能会因为网络问题导致服务器收不到，服务器会再次发送  ``` FIN ```  包，如果这时客户端完全关闭了连接，那么服务器无论如何也收不到 ``` ACK ``` 包了，所以客户端需要等待片刻、确认对方收到 ``` ACK ``` 包后才能进入CLOSED状态。那么，要等待多久呢？
